...
class YouTubeDownloaderApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # URL
        self._field_label(inner, "URL PLAYLIST SPOTIFY").grid(
        ...
        self._pill_btn(url_row, "Incolla", self._paste_text).grid(row=0, column=1)

        # Output dir
        self._field_label(inner, "CARTELLA DI DESTINAZIONE").grid(
        ...
        self._make_entry(dir_row, self._dir_var).grid(
            row=0, column=0, sticky="ew", ipady=9, padx=(0, 8))
        self._pill_btn(dir_row, "Sfoglia", self._browse_dir).grid(row=0, column=1)

        # Qualità
        qual_outer = tk.Frame(inner, bg=BG2)
        qual_outer.grid(row=4, column=0, sticky="w")
        self._field_label(qual_outer, "QUALITÀ AUDIO").pack(anchor="w", pady=(0, 5))
    ...
    def _check_deps(self):
        ...
        if ok_ytdlp and ok_ffmpeg:
            ...
            msgs = []
            if not ok_ytdlp:
                msgs.append("yt-dlp non trovato  → scarica da https://github.com/yt-dlp/yt-dlp/releases/download/2026.03.03/yt-dlp.exe e sposta su C:\Windows\System32, poi scarica python con winget install python")
            if not ok_ffmpeg:
                msgs.append("ffmpeg non trovato  →  winget install ffmpeg")
    ...
    def _start_download(self):
        ...
        if not url:
            messagebox.showwarning("URL mancante", "Inserisci l'URL di una playlist Spotify.")
            return
        if "spotify.com" not in url or "playlist" not in url:
            messagebox.showwarning("URL non valido", "L'URL deve essere un link a una playlist Spotify.")
            return
        if not out:
        ...
        threading.Thread(target=self._extract_and_download,
                         args=(url, out, self._qual_var.get()),
                         daemon=True).start()

    def _extract_and_download(self, spotify_url, out, quality):
        """Estrae le canzoni da Spotify e le scarica da YouTube"""
        try:
            # Step 1: prova l'embed veloce
            self.after(0, lambda: self._log_write("🔍 Leggo la playlist dalla pagina Spotify…", "dim"))
            songs = self._get_songs_requests(spotify_url)

            if songs and len(songs) >= 100:
                # Potrebbe essere troncata a 100 — usa spotdl per avere la lista completa
                self.after(0, lambda: self._log_write(
                    "⚠ Playlist con 100+ tracce — uso spotdl per lista completa…", "warn"))
                full = self._get_spotify_songs_spotdl(spotify_url)
                if full:
                    songs = full

            if not songs:
                # Fallback diretto a spotdl
                self.after(0, lambda: self._log_write("🔄 Fallback: uso spotdl save…", "dim"))
                songs = self._get_spotify_songs_spotdl(spotify_url)
    ...
    def _get_songs_requests(self, spotify_url):
        """
        Estrae le canzoni dalla pagina embed di Spotify (no API key).
        L'embed pre-renderizza i metadati in un tag <script> come JSON.
        """
        try:
            import urllib.request

            # Converti URL normale in URL embed
            ...
            self.after(0, lambda: self._log_write(f"   📡 scarico pagina embed Spotify…", "dim"))
            ...
            # Cerca il tag script con il JSON dei dati della playlist
            ...
            if not track_list:
                self.after(0, lambda: self._log_write("⚠ trackList vuota nel JSON", "warn"))
                return None

            songs = []
            for t in track_list:
                title    = t.get("title", "").strip()
                subtitle = t.get("subtitle", "").strip()  # artisti separati da virgola
            ...
            if songs:
                ...
                for i, s in enumerate(songs, 1):
                    self.after(0, lambda i=i, s=s: self._log_write(f"   {i:>2}. {s}", "dim"))
                return songs

            return None

        except Exception as e:
            self.after(0, lambda e=e: self._log_write(f"⚠ Errore scraping: {e}", "warn"))
            return None
